...
namespace AdbFileManager
{
    public partial class Form1 : Form
    {
        ...
        private void btnDeleteMsg_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("You have to select something first before attempting to delete it", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            DialogResult result = MessageBox.Show("Delete the selected files?", "Confirmation", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);

            if (result == DialogResult.OK)
            {
                int filecount = dataGridView1.SelectedRows.Count;
                int copied = 0;
                Form2 progressbar = new();
                progressbar.Show();
                progressbar.BringToFront();
                progressbar.Activate();
                progressbar.Focus();
                ...
                copying = true;
                // TODO: optimize for multithreading (use the linq thingy)
                foreach (DataGridViewRow row in dataGridView1.SelectedRows)
                {
                    string sourceFileName = Convert.ToString(row.Cells[1].Value);
                    progressbar.update(copied, filecount, directoryPath, "trash", Convert.ToString(row.Cells[1].Value));
                    string sourcePath = directoryPath + sourceFileName;
                    string command = $"adb shell rm -vRrf \"{sourcePath}\"";
            ...
            // force update because yes
            verticalLabel1_Click(this, EventArgs.Empty);
        }
        ...
        {
            // if double clicked the title bar, that means we are maximizing/minimizing the window, and we need to redraw the controls.
            if (m.Msg == WM_NCLBUTTONDBLCLK)
            {
                base.WndProc(ref m);
                this.OnForm1ResizeEnd(this, EventArgs.Empty);
                return;
            }
            base.WndProc(ref m);
        }

        private void OnForm1ResizeEnd(object sender, EventArgs e)
        {
            // we are going to calculate using constants and calculating the width / height of the form1, which is the main window.
            ...
            cur_path.Width = dataGridView1.Size.Width + 27;
            cur_path.Invalidate();

            panel2.Location = new Point(0, height - 51 + 5);
            panel2.Width = width;
            panel2.Invalidate();

            //panel2.Location = new Point(0, height - 51);
            //panel2.Invalidate();

            android2pc.Location = new Point(
                explorerBrowser1.Location.X - buttonWidth - 8,
                ((int)Math.Floor((double)height / 2)) - (buttonHeight + 4)
            );
            android2pc.Invalidate();
            pc2android.Location = new Point(
                explorerBrowser1.Location.X - buttonWidth - 8,
                ((int)Math.Floor((double)height / 2))
           );
            pc2android.Invalidate();
            btnDeleteMsg.Location = new Point(
                explorerBrowser1.Location.X - buttonWidth - 8,
                ((int)Math.Floor((double)height / 2)) + (buttonHeight + 4)
            );
            btnDeleteMsg.Invalidate();
        }
    }
